...
def resolve_model_path():
    ...
    search_roots = []
    for root in [BASE_DIR, Path.cwd(), BASE_DIR.parent, Path.cwd().parent, Path('/opt/render/project/src')]:
...
def get_model():
    ...
    if resolved_path is None:
        ...
        model_load_error = (
            ...
            f"BASE_DIR={BASE_DIR}. CWD={Path.cwd()}. Files visible to app: {visible_files[:80]}"
        )
        return None
    ...
    try:
        ...
        print(f"Model load failed: {model_load_error}", flush=True)
        return None
...
def healthz():
    ...
    return jsonify({
        ...
        'cwd': str(Path.cwd()),
        'model_found': resolved_path is not None,
